...
for (i in 1:500){
  ...
  mymethods = c("rf") # Data mining method
  ...
  if (i > 1){
    mod <- rbind(mod,out$rf$bestTune)
  }else{
    mod <- data.frame(out$rf$bestTune)
  }
...
write.csv(mod,'Error_2n_10_MNAR_RF_OUT.csv') # CSV file with parameters
