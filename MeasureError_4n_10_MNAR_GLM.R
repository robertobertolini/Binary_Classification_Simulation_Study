...
for (i in 1:500){
  ...
  # Class Imbalance: 10% Minority Class and 90% Majority Class
  ...
  mymethods = c("glm") # Data mining method
  ...
  if (i > 1){
    mod <- rbind(mod,out$glm$bestTune)
  }else{
    mod <- data.frame(out$glm$bestTune)
  }
...
write.csv(mod,'Error_4n_10_MNAR_GLM_OUT.csv') # CSV file with parameters
